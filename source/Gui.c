...
#define EMUVERSION "V0.2.4 2022-10-04"
...
const fptr fnList8[] = {difficultSet, coinASet, coinBSet, livesSet, bonusSet, demoSet, cabinetSet, flipSet, stereoSet};
...
void uiAbout() {
	...
	drawMenuText("DPad:   Move vehicle",6,0);
	drawMenuText("B:      Normal shot",7,0);
	drawMenuText("A:      Lob grenade",8,0);

	drawMenuText("JackalDS     " EMUVERSION, 20, 0);
	drawMenuText("ARM6809      " ARM6809VERSION, 21, 0);
	drawMenuText("ARMK005849   " K005849VERSION, 22, 0);
	drawMenuText("ARMYM2151    " ARMYM2151VERSION, 23, 0);
}
...
void uiDipswitches() {
	...
	drawSubItem("Cabinet:", cabTxt[(gDipSwitch3>>6)&1]);
	drawSubItem("Flip Screen:", autoTxt[(gDipSwitch3>>5)&1]);
	drawSubItem("Sound Mode:", stereoTxt[(gDipSwitch3>>7)&1]);
...
void uiLoadGame() {
	...
	int i;
	for (i=0; i<ARRSIZE(jackalGames); i++) {
		drawSubItem(jackalGames[i].fullName, NULL);
	}
}
